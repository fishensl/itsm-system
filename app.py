...
from models import (AIConfig, User as UserM, UserPermission, Permission, Topology, Department)
from sqlalchemy.orm import joinedload
from utils.permission import admin_required

@app.route('/topologies', methods=['GET', 'POST'])
@login_required
...
def topology_list():
    if request.method == 'POST':
        ...
        if f and f.filename:
            ...
            import os as _os
            upload_dir = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), 'static', 'uploads', 'topologies')
            _os.makedirs(upload_dir, exist_ok=True)
            safe_name = secure_filename(f.filename)
            f.save(_os.path.join(upload_dir, safe_name))
            # 创建一条 Topology 记录（含客户/地区关联）
            t = Topology(
                ...
                file_type='image' if safe_name.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp', '.webp')) else 'other',
    ...
    # 地区：包含所有地市 + 区/县，按层级排序
    regions = Region.query.order_by(Region.parent_id.is_(None).desc(), Region.parent_id, Region.sort_order, Region.id).all()
    # V6 增强：服务端按客户分组，"未关联客户" 排最后
...
@app.route('/topologies/upload', methods=['POST'])
@login_required
...
def topology_upload():
    ...
    # 保存到 static/uploads/topologies/
    upload_dir = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)),
                                'static', 'uploads', 'topologies')
    _os.makedirs(upload_dir, exist_ok=True)
    safe_name = secure_filename(f.filename) or ('topology' + ext)
    # 防止重名覆盖：加时间戳
    import time
    base, e = _os.path.splitext(safe_name)
    safe_name = f"{base}_{int(time.time())}{e}"
    full_path = _os.path.join(upload_dir, safe_name)
    f.save(full_path)
