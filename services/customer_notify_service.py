...
from services.notification_outbox import CUSTOMER_EVENTS as EVENT_LABELS, insert_event, CANCELLED, PENDING, RETRY
...
def settings(customer_id):
    ...
    return dict(notify_enabled=any(r.enabled for r in rows), has_wecom_webhook=any(r.webhook_encrypted for r in rows),
...
def save_binding(customer_id, secret, enabled, *, binding_id=None, create=False, channel_type='wecom',
                 name='客户群', signing_secret='', subscriptions=None, quiet_start=0, quiet_end=0, digest_minutes=0):
