...
- 设备 Excel 模板增加当前客户/类型/品牌/网络/电源等下拉及填写说明；导入升级为预检、网络类型显式映射、仅新增/仅更新/upsert、空值语义、错误明细、U 位冲突校验、幂等批次和同客户同名数据库约束。
...
- 发布脚本在迁移前执行设备重名和机柜功率冲突只读门禁并保存 JSON 报告；新增额定功率、电源字典、拓扑模板版本、导入批次和设备唯一约束迁移。
