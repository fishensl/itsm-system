...
DEVICE_FIELDS = (
    ...
    F('is_in_use', '是否在用', data_type='boolean', width=90, filterable=True,
      value_map={'true': '是', 'false': '否'}),
    F('pwd_changed_by', '上次修改账号', min_width=110, group='audit', default_visible=False),
    F('pwd_changed_at', '上次修改时间', data_type='datetime', min_width=130,
...
DEVICE_EXPORT_PRESETS = {
    'asset': ('customer_name', 'rack_location', 'rack_name', 'location', 'rack_slot',
              ...
              'ip_address', 'interface', 'build_date',
    ...
    'version': ('customer_name', 'rack_location', 'device_name', 'os_version', 'rule_version',
                'device_type', 'brand', 'model', 'serial_number', 'ip_address',
                'license_start', 'license_expiry', 'build_date', 'is_in_use', 'remark'),
...
INSPECTION_FIELDS = (
    ...
    F('task_contract_period', '合同时效', width=210, group='task_timing'),
    F('task_deadline_period', '任务期限', width=210, group='task_timing'),
    F('task_actual_start', '实施开始', data_type='datetime', width=140,
      group='task_timing'),
    F('task_actual_end', '实施结束', data_type='datetime', width=140,
      group='task_timing'),
    F('task_actual_duration', '实施耗时', width=120, group='task_timing'),
    F('task_actual_effort', '实施人天', data_type='number', width=110,
      group='task_timing'),
...
INSPECTION_TASK_FIELDS = (
    ...
    F('planned_start', '合同时效开始', data_type='date', width=120),
