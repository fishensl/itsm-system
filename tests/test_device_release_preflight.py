"""设备覆盖导入发布检查（同名资产允许，更新时必须使用设备 ID）。"""
...
def test_duplicate_name_is_reported_without_blocking_release(tmp_path):
    report = collect_report(_legacy_database(tmp_path, assigned=True))
    assert report['gate_passed'] is True
    assert report['overwrite_requires_device_id'] is True
...
def test_unassigned_same_name_does_not_block_id_only_update_rule(tmp_path):
    ...
    assert report['gate_passed'] is True
    assert report['overwrite_requires_device_id'] is False
