...
def test_mfa_setup_confirm_and_status(op_client, app):
    ...
    assert unquote(parsed_uri.path.lstrip('/')) == 'ITSM · 账号（op）'
    ...
    status = op_client.get('/api/auth/mfa/status').get_json()['data']
    assert status['account_enabled'] is True
    assert status['login_enabled'] is True
    assert status['operation_enabled'] is True
    assert status['operation_uses_account_mfa'] is True
    assert op_client.get('/api/auth/me').get_json()['data']['mfa_enabled'] is True

    # An older cached SPA may still request purpose=operation. It must resolve
    # to the existing account binding, never create a second authenticator.
    duplicate = op_client.post('/api/auth/mfa/setup', json={'purpose': 'operation'})
    assert duplicate.status_code == 400
    assert duplicate.get_json()['message'] == '账号验证器已绑定，请使用换绑流程'

    from models import User
    with app.app_context():
        user = User.query.filter_by(username='op').first()
        assert user.mfa_secret_encrypted
        assert user.mfa_op_secret_encrypted is None
        assert user.mfa_op_enabled is False
...
def test_operation_code_token_and_default_off_guard(op_client, app):
    ...
    setup = op_client.post('/api/auth/mfa/setup', json={'purpose': 'login'}).get_json()['data']
    code = pyotp.TOTP(setup['manual_secret']).now()
    assert op_client.post('/api/auth/mfa/confirm',
                          json={'purpose': 'login', 'code': code}).status_code == 200
    ...
    with app.app_context():
        ...
        secret = __import__('utils.crypto', fromlist=['decrypt_password']) \
            .decrypt_password(user.mfa_secret_encrypted)
        assert user.mfa_op_secret_encrypted is None
        assert user.mfa_op_enabled is False
...
def test_operation_failures_persist_and_lock(op_client, app):
    setup = op_client.post('/api/auth/mfa/setup', json={'purpose': 'login'}).get_json()['data']
    code = pyotp.TOTP(setup['manual_secret']).now()
    op_client.post('/api/auth/mfa/confirm', json={'purpose': 'login', 'code': code})
    for _ in range(5):
...
def test_admin_mfa_reset_increments_auth_version(admin_client, app):
    ...
    with app.app_context():
        ...
        assert target.mfa_op_enabled is False
        assert target.mfa_op_secret_encrypted is None
        assert target.auth_version == before + 1


def test_legacy_operation_setup_is_normalized_to_account_mfa(op_client, app):
    setup = op_client.post('/api/auth/mfa/setup', json={'purpose': 'operation'})
    assert setup.status_code == 200
    data = setup.get_json()['data']
    assert data['purpose'] == 'login'
    assert unquote(urlparse(data['provisioning_uri']).path.lstrip('/')) == 'ITSM · 账号（op）'

    code = pyotp.TOTP(data['manual_secret']).now()
    confirmed = op_client.post('/api/auth/mfa/confirm', json={
        'purpose': 'operation', 'code': code,
    })
