...
from blueprints.ops import reports as ops_reports


class TestSafeReportPath:
    def test_traversal_rejected(self, app, tmp_path):
        with app.app_context():
            assert ops_reports._safe_report_path('../../etc/passwd.docx') is None
            assert ops_reports._safe_report_path('..\\..\\x.docx') is None
            assert ops_reports._safe_report_path('') is None

    def test_bad_extension_rejected(self, app):
        with app.app_context():
            assert ops_reports._safe_report_path('evil.exe') is None
            assert ops_reports._safe_report_path('x.txt') is None

    def test_missing_file_rejected(self, app):
        with app.app_context():
            assert ops_reports._safe_report_path('not-exists.docx') is None
    ...
    def test_valid_file_accepted(self, app, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(ops_reports, 'REPORTS_DIR', str(reports))
        with app.app_context():
            got = ops_reports._safe_report_path('巡检报告_A.docx')
