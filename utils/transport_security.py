...
def _allows_trusted_internal_http() -> bool:
    """允许可信内网 HTTP 的有限兼容操作。

    新用户使用临时密码登录后必须立即自助改密；若登录允许内网 HTTP、改密却强制
    HTTPS，会导致首次登录流程无法闭环。这里只放行本人自助改密和既有设备凭据
    兼容端点，管理员重置他人密码、密码导出等高风险操作仍要求 HTTPS。
    """
    path = request.path.rstrip('/')
    return (path == '/api/auth/change-password' or
            '/reveal-password' in path or _contains_password_write())


def register_transport_security(app):
    """Reject credential-bearing HTTP in production.

    新用户本人首次改密及设备凭据接口兼容已配置的可信内网/VPN：外网 HTTP
    仍拒绝；管理员重置、密码导出等其他高风险操作仍必须使用 HTTPS。
    """
    @app.before_request
    def require_https_for_credentials():
        ...
        if _is_sensitive_request():
            if _allows_trusted_internal_http():
                try:
                    from utils.access_control import is_internal_request
                    if is_internal_request():
                        return None
                except Exception:
                    app.logger.exception('敏感操作可信网段判定失败')
