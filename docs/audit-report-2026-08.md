...
> 实施更新：截至 2026-08-21，P0/P1、确定性 P2/P3、G2 主密钥轮换和 G3 隔离恢复均已完成。G1 已改写并强推 `master` 与全部发布标签，本地和生产仓库均已清除旧对象；生产仓库当前为新历史 `48db119`，服务、备份 timer 和 `/readyz` 正常。GitHub 仍允许按已知 SHA 获取已失联 blob，平台侧垃圾回收待 Support 处理，详见 `docs/github-sensitive-data-removal-request-2026-08-21.md`。
> 2026-08-28 增量：提交 `5d7dc04` 已完成客户任意层级、设备字段/导入导出、额定功率、机柜完整性、拖拽排序和标准拓扑模板等代码实施；本轮未向生产发布，生产数据预检、层级映射和迁移演练仍属于发布门禁。
>
> **提交编号说明**：本报告保留的 `ece64de`、`507c2fd` 等编号是 G1 前的审计证据编号；历史改写后编号已变化或被剪枝，主要映射见 `docs/git-history-rewrite-notice-2026-08-21.md`。
...
- 审计基线核对 229 个 Python 文件、45 个 Vue 视图、36 个迁移文件、64 个测试文件，当时可收集 840 个 pytest 用例；整改持续增加回归后 2026-08-28 当前收集数为 1000。
...
- CI 已覆盖 Python 3.10/3.12、pytest、Ruff、前端 lint/unit/build；迁移不可变、状态生成物、metadata 权限矩阵、JSON 边界和 UI 语义色均有门禁，2026-08-28 当前 1000 个测试全量通过（`.github/workflows/ci.yml`；`tests/test_entity_metadata.py`；`tests/test_json_boundaries.py`；`tests/test_ui_convergence.py`）。

### 1.4 当前状态标记

| 项目 | 当前状态 | 证据/说明 |
|---|---|---|
| 旧 Fernet 主密钥备份退出当前索引 | **已修复** | G1 后全部分支/标签历史已移除 `.secret.key.bak.20260719_165843`；原删除提交 `ece64de` 因变为空提交被剪枝 |
| 旧密钥 blob 从 Git 历史移除 | **仓库 refs 已完成，GitHub GC 待处理** | 本地、全新 bundle 克隆和生产仓库均为路径命中 0、旧 blob 不可解析；远端 `master` 和 4 个发布标签已强推。GitHub 仍允许按旧 SHA 直接 fetch，须由 GitHub Support 清除悬空对象和缓存 |
| 当前生产密钥轮换 | **已完成（G2）** | 停服前后均生成并校验配对备份；`--apply` 将 1 条 MFA 密文重加密并全量回读通过，当前密钥与泄露密钥指纹不同、权限为 `600`；设备密码、AI Key、通知凭据当前均无密文样本 |
...
| 客户层级、设备、机柜与拓扑治理增量 | **代码已完成，生产待发布门禁** | `5d7dc04`：客户任意层级及路径导入导出；设备额定功率、统一字段顺序、机房筛选、批量更新/删除和原子导入；机柜 U 位/功率完整性；网络/会议标准拓扑；多类设置拖拽排序。生产预检、数据映射和部署尚未执行 |
...
| **G2/G3 完成；G1 平台清理待办** | DS-01 / G1、G2、G3 | G3 已通过；G2 已轮换并生成新旧密钥各自的配对备份；G1 分支、标签、本地和生产对象清理已完成，仅剩 GitHub Support 对悬空 blob 的平台垃圾回收 |
...
- **G3 隔离恢复**：以 `itsm_pg_20260821_053645.dump` 和同时间戳 meta 包建立独立 PostgreSQL 数据库，显式复用生产 `UTF8/C/C` 编码。64 张表行数与生产零差异，103 个归档文件全部解包，密钥权限为 `600`；迁移为 `b0c1d2e3f4a5`，MFA 密文 1/1 可解，隔离探针登录和 `/readyz` 均为 HTTP 200。生产当前设备密码、AI Key、通知凭据密文均为 0 条，属于无样本而非解密失败。演练结束后临时数据库和目录均已删除，详细证据见 `docs/recovery-drill-2026-08-21.md`。
- **G2 主密钥轮换**：停服前备份为 `itsm_pg_20260821_054731.dump` 与配对 meta；轮换后备份为 `itsm_pg_20260821_054812.dump` 与配对 meta。旧密钥和停服前备份存放于 root-only 托管目录 `backups/key-rotation-20260821_054739`（目录 `700`、文件 `600`）；新密钥权限为 `600`，1/1 条 MFA 密文可解，服务和 timer 恢复 active，`/readyz` 为 ready。
- **G1 历史改写**：`master` 从旧 `41e2730` 映射为新 `48db119`，最终 tree 保持 `23a1f9b...` 不变；4 个发布标签同步强推，无开放 PR refs。本地与生产仓库均完成 reflog 过期和 GC，泄露路径命中为 0、旧 blob 不可解析。生产遗留 stash 在排除密钥文件后归档 422 个工作树文件和 472 个未跟踪文件，再删除旧 ref。GitHub 平台仍可按旧 blob SHA 获取对象，已形成 Support 清理申请。

### 1.7 客户层级、设备、机柜与拓扑治理增量（2026-08-28，`5d7dc04`）

| 范围 | 状态 | 已实现内容 | 尚未执行/待确认 |
|---|---|---|---|
| 客户层级 | **代码已完成** | `parent_id` 任意层级、路径显示与导入导出、跨地区挂接、循环引用与删除保护、树形 UI、预览/应用归一化脚本 | 生产上下级单位映射须由业务确认后运行预览，再单独批准 `--apply` |
| 设备字段与导入导出 | **代码已完成** | 列表/详情/编辑/批量/模板/导出统一字段；新增额定功率；机房位置/机柜号/安装位置/U 位连续；设备资产表、密码表、安全版本表统一；机房筛选与导出筛选一致 | “内网”等历史网络类型不自动新增为永久字典，导入时使用显式映射；生产同名设备与功率冲突须先跑只读检查 |
| 设备批量导入 | **代码已完成** | create/update/upsert、设备 ID 或无歧义时“客户+名称”匹配、清空语义、未知网络类型汇总与映射、错误工作簿、整批原子预检、幂等批次；同客户允许多台同名设备，歧义覆盖必须填写设备 ID | 2026-08-29 生产检查发现 7 组同客户同名设备均为真实资产，已撤销错误唯一约束设计，禁止以清理真实设备换取迁移通过 |
| 设备字典与排序 | **代码已完成** | 单/双/四电源配置字典；设备类型、品牌、网络类型、电源、客户类别、角色、地区、故障分类、部门支持拖拽排序，移动端保留上下移动；字典改名同步引用 | 是否增加更多电源枚举由业务维护字典，无需再改代码 |
| 机柜管理 | **代码已完成** | 简表固定为“起始U位、名称、品牌、型号、IP、额定功率”；机柜位置作为已上架设备机房位置单一真源；U 位冲突、越界、容量缩小和跨客户变更保护；功率缺失/PDU/BTU 指标 | UPS 容量系数、冗余策略和功率因数尚无确认口径，因此未写入推荐算法，只展示原始 W、PDU 与缺失告警 |
| 拓扑标准模板 | **代码已完成** | 网络/会议两套模板、真实连线图例和颜色规范、模板版本、客户范围、旧空白拓扑显式插入模板、XML 安全校验 | 需在生产发布后由用户抽查 Draw.io 打开、保存、导出和旧图兼容性 |
| 验证 | **已通过** | PostgreSQL 全量 pytest 1000 项、前端 unit 42 项、Ruff、diff check、lint 0 error、生产构建均通过 | lint 仍有 17 条既有 warning；Vite 仍提示大 chunk，均非本次新增阻断项 |
| 发布 | **准备完成，生产未执行** | `origin/master` 已同步；离线 bundle/前端包及 SHA256 manifest 已生成并校验。2026-08-07 旧生产 dump 从 `c9d0e1f2a3b4` 升级到 `a5b6c7d8e9f0` 成功 | 旧 dump 的设备表为 0 条，不能替代当前设备数据门禁；2026-08-29 生产 SSH 不可达，仍须完成当前生产只读预检、配对备份和备份副本迁移演练后再进入维护窗口 |
...
> **实施状态：代码项已完成；G2 已完成，G1 仓库历史改写已完成，GitHub 平台悬空对象清理待 Support。**
...
> **实施状态：P1 代码及 G3 隔离恢复均已完成。生产关系审计确认 `customer_engineers` 为 0 条，数据 scope 强制开关继续保持关闭，补齐关系后再启用。**
...
| 故障工单自动生成故障报告 | 当前工单提交已允许不上传故障报告，但尚无按工单数据生成正式报告的能力 | 提交前可预览/编辑并生成 DOCX；取工单、设备、处置时间线、诊断、方案、照片和审核信息；手工上传与系统生成并存且不相互覆盖；版本、审计、权限和导出资料包完整 | P2 |
...
### 9.1 故障工单自动生成故障报告单（P2 实施方案）

产品边界：故障报告始终是**可选附件**，无报告不得阻断工单提交、审核或关闭；自动生成只是减少人工整理，不替代工程师确认，也不得覆盖手工上传文件。

当前状态：巡检资料补传、资产导入事务回滚以及故障报告改为可选已由 `87c8135` 实施；以下“自动生成故障报告单”仍是待实施 P2，不得误标为已完成。

1. **报告数据契约**：固定取工单号、客户、关联设备、三级故障分类、严重级别/影响范围、报障信息、派单/接单/完成时间、处置进展与照片、诊断、根因、解决方案、状态日志、审核和验收信息；缺失项显示“未填写”，不能编造。
2. **文件与版本模型**：新增 `ticket_report_artifacts`（`ticket_id`、可空 `submission_version_id`、`source=uploaded/generated`、`template_version`、`file_path`、`sha256`、`is_current`、创建人/时间）。保留 `Ticket.report_file` 作为兼容的当前报告指针；手工上传和系统生成分别留档、可切换当前版本、不可覆盖历史文件。
3. **生成服务**：新增 `services/ticket_report_service.py` 和 `utils/ticket_report_generator.py`，复用现有 `python-docx`；模板按版本管理，生成到私有上传目录，临时文件写完后原子替换，文件名使用“客户+工单号+故障报告+版本”。
4. **API 与 UI**：新增 `POST /api/tickets/<id>/reports/generate`（`ticket:edit`）和报告列表/预览/下载接口（`ticket:view`）；工单详情增加“生成故障报告”，先预览数据快照，再确认生成；提交审核页仍保留“无报告直接提交”和“上传现有报告”。
5. **审核、审计与导出**：生成、重生成、切换当前版本和下载均写 `audit_logs`；提交版本关联当时选定的报告 artifact；报告包同时包含手工/生成文件并标注来源，不以“最新文件”静默替换历史。
6. **验收门禁**：覆盖无报告完整闭环、生成/重生成、缺字段、照片损坏、并发生成、越权访问、路径穿越、版本回溯、报告包导出和 DOCX 打开/内容断言；生成失败不改变工单状态和现有报告指针。
...
- `.venv/Scripts/python -m pytest tests/`：**1000 项全量通过**，退出码 0；使用真实 PostgreSQL 迁移链路，覆盖空库升级、新唯一/检查约束、导入原子性、客户范围、机柜和拓扑模板。
- `.venv/Scripts/python -m ruff check .`：**通过**，`All checks passed!`；`git diff --check`：**通过**。
- 前端 `npm run lint`：**通过**（17 条既有 warning、0 error）；`npm run test:unit`：**42 项通过**；`npm run build`：**通过**。
- Git Bash `bash -n`：`backup.sh`、`update.sh`、`lib-release.sh` **全部通过**；发布恢复 3 条故障注入测试全部通过。
- 测试 PostgreSQL 每次运行使用随机目录，避免中断后的 `postmaster.pid` 污染后续运行。
- 生产主机、systemd、定时任务、备份状态和密钥权限已于 2026-08-21 核验；`5d7dc04` 增量尚未完成生产只读预检、备份副本迁移演练、远端同步和部署。

## 11. 需要单独确认的高风险操作

1. **Git 历史改写**：2026-08-21 已执行；协作者必须重新克隆，禁止把旧历史 merge/push 回远端。GitHub 悬空对象仍待 Support 清理。
2. **主密钥轮换 `--apply`**：2026-08-21 已执行并通过；旧备份只能与 root-only 托管的旧密钥配对恢复，日常新备份使用新密钥。
