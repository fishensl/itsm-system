...
| 设备字段与导入导出 | **代码已完成** | 列表/详情/编辑/批量/模板/导出统一字段；新增额定功率；机房位置/机柜号/安装位置/U 位连续；设备资产表、密码表、安全版本表统一；机房筛选与导出筛选一致 | “内网”等历史网络类型不自动新增为永久字典，导入时使用显式映射；生产重复设备与功率冲突须先跑只读预检 |
| 设备批量导入 | **代码已完成** | create/update/upsert、设备 ID 或“客户+名称”匹配、清空语义、未知网络类型汇总与映射、错误工作簿、整批原子预检、幂等批次；唯一约束迁移前置检查 | 生产备份副本上的重复数据清理与迁移演练未执行；存在重复时发布脚本会中止，不会强行迁移 |
...
| 发布 | **准备完成，生产未执行** | 已同步 `origin/master@57172a7`；离线 bundle/前端包及 SHA256 manifest 已生成并校验。2026-08-07 旧生产 dump 从 `c9d0e1f2a3b4` 升级到 `a5b6c7d8e9f0` 成功 | 旧 dump 的设备表为 0 条，不能替代当前设备数据门禁；2026-08-29 生产 SSH 不可达，仍须完成当前生产只读预检、配对备份和备份副本迁移演练后再进入维护窗口 |
