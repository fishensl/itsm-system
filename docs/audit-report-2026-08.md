...
- `.venv/Scripts/python -m pytest tests/`：**本次全量运行 100% 通过**，退出码 0；全量启动后新增的资产导入事务回滚用例又随 21 项相关回归通过，当前收集 **958 项**。告警均为既存弃用/兼容性告警，无测试失败。
- `frontend npm run test:unit`：**40 项通过**；`npm run build` 通过；`.venv/Scripts/python -m ruff check .` 通过。
